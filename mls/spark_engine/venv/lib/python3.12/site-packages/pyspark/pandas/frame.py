...
def _reduce_for_stat_function(
    self,
    sfun: Callable[["Series"], Series],
    numeric_only: bool = False,
) -> DataFrame:
    """
    Apply a function to each column and return a DataFrame with the results.

    Parameters
    ----------
    sfun : function
        A function that takes a Series and returns a PySpark Column.
    numeric_only : bool, optional
        If True, only apply the function to numeric columns.

    Returns
    -------
    DataFrame
        A DataFrame with the results of applying the function to each column.
    """

class DataFrame(DataFrame):
from pyspark.pandas import DataFrame        super().__init__(data, index, columns, dtype, copy)
    ...
    def select_dtypes(self, include=None, exclude=None):
        if include is not None:
            return [
                ...
                if self.data[col].dtype in include
            ]
        elif exclude is not None:
            return [
                col for col in self.data.columns
                if self.data[col].dtype not in exclude
            ]
        else:
            return self.data.columns

    def select(self, columns):
        return DataFrame(self.data[columns])

def process_dataframe(df, numeric_only=False):
    if numeric_only:
...
processed_df = process_dataframe(df, numeric_only=True)
print(processed_df.data)
...
def apply_function_to_dataframe(df: DataFrame, sfun: Callable[[col], col]) -> DataFrame:
    """
    Apply a function to each column of a DataFrame and return a new DataFrame with the transformed columns.

    Parameters
    ----------
    df : DataFrame
        The input DataFrame.
    sfun : Callable[[col], col]
    ...
    """
    return df.select(*[sfun(col(column).alias(column)) for column in df.columns])
import warnings
import inspect
import json
import types
...
class DataFrame(Frame, Generic[T]):
    ...
    def __init__(  # type: ignore[no-untyped-def]
        self, data: object = None, index: object = None, columns: object = None, dtype: object = None, copy: object = False
    ) -> object:
        global internal
    ...
    # Arithmetic Operators
    def _map_series_op(self, op: str, other: Any,
                       self, self, other) -> "DataFrame":
    ...
    def corr(self, internal, internal, internal, internal, internal, method: str = "pearson", min_periods: Optional[int] = None) -> "DataFrame":
        """
        ...
        for label in numeric_labels:
            numeric_scols.append(internal.spark_column_for(label).cast("double"))
    ...
    def corrwith(
        ...
        if method not in ["pearson", "spearman", "kendall"]:
            raise ValueError(f"Invalid method {method}")
        if not isinstance(other, (DataFrame, pyspark.pandas.series.Series)):
            raise TypeError("unsupported type: {}".format(type(other).__name__))

        right_is_series = isinstance(other, pyspark.pandas.series.Series)
        ...
        sser = pyspark.pandas.series.first_series(DataFrame(internal))
        sser.name = None
        return sser
    ...
    def to_dict(self, orient: str = "dict", into: Type = dict) -> Union[List, collections.abc.Mapping]:
        """
...
defaultdict(<class 'list'>, {'col..., 'col...})]
    ...
    def transpose(self, self) -> "DataFrame":
        """
    ...
    _parse_datatype_string=None, _parse_datatype_string=None) -> "DataFrame":
        """
        ...
        * To select datetimes, use ``np.datetime64``, ``'datetime'`` or
        ...
        4  1   True  1.0
        5  2  False  2.0
        @type _parse_datatype_string: object
        """
    ...
    def melt(
        self,
        self, self, self, id_vars: Optional[Union[Name, List[Name]]] = None,
    ...
    def unstack(self, self) -> DataFrameOrSeries:
        """
    ...
    def rename(
        ...
        global index_mapper_ret_dtype, index_mapper_ret_dtype
    ...
    def __setitem__(self, key: Any, value: Any, self) -> None:
        from pyspark.pandas.series import Series
...
def _reduce_spark_multi(sdf: PySparkDataFrame, aggs: List[PySparkColumn], iloc=None) -> Any:
    """
