...
from pyspark.sql.utils import _parse_datatype_json_string
from pyspark.sql.types import StructType

# ...

return_type: StructType = _parse_datatype_json_string(utf8_deserializer
                                                      .loads(infile
                                                             ))
...
def read_udtf(pickleSer, infile, eval_type):
    ...
    if not isinstance(return_type, StructType):
        raise PySparkRuntimeError(
            "The return type of a UDTF must be a struct type, but got {type(return_type).__name__}."
        )

    # Instantiate the UDTF class.
    try:
        udtf = object()  # Replace this with the actual instantiation of the UDTF class
    ...
    if not hasattr(udtf, "eval"):
        raise PySparkRuntimeError(
            "Failed to execute the user-defined table function because it has not implemented the 'eval' method. Please add the 'eval' method and try the query again."
        )
...
_accumulatorRegistry = AccumulatorRegistry.get()
...
def read_udtf(pickleSer, infile, eval_type):
    # ... rest of the code ...    def evaluate(*args: pd.Series) -> pd.Series:
    from scripts.run_emscripten_tests import args
...
def read_single_udf(pickleSer, infile, eval_type, runner_conf, udf_index):
    global return_type, return_type, return_type, return_type, return_type, return_type, return_type, return_type, return_type, return_type, return_type
    ...
    if eval_type in (
        ...
        func = pyspark.sql.utils.fail_on_stopiteration()
...
def read_udtf(pickleSer, infile, eval_type):
    ...
    if not isinstance(handler, type):
        raise PySparkRuntimeError(
            "Invalid UDTF handler type. Expected a class (type 'type'), but "
            "got an instance of {type(handler).__name__}."
        )

    return_type = _parse_datatype_json_string(utf8_deserializer.loads(infile))
    if not type(return_type) == StructType:
        raise PySparkRuntimeError(
            "The return type of a UDTF must be a struct type, but got {type(return_type)}."
        )
...
def main(infile, outfile, _broadcastRegistry=None):
    global faulthandler_log_file, broadcast_sock_file
    ...
    try:
        ...
        taskContext._taskAttemptId = read_long()
        ...
        needs_broadcast_decryption_server = read_bool()
        ...
        for _ in range(num_broadcast_variables):
            ...
            if bid >= 0:
                if needs_broadcast_decryption_server:
                    ...
                    _broadcastRegistry[bid] = Broadcast_broadcastRegistry[bid] = Broadcast(sock_file=broadcast_sock_file)
read_udtf(pickleSer, infile, eval_type)sock_file=broadcast_sock_file)
                else:
...
_broadcastRegistry[bid] = Broadcast(path=path)
        ...
        try:
            ...
            if os.environ.get("SPARK_SIMPLIFIED_TRACEBACK", False):
                tb = pyspark.sql.utils.try_simplify_traceback()
                if tb is not None:
                    e.__cause__ = None
